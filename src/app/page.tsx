      ...
      <section className="relative min-h-screen flex items-center justify-center overflow-hidden">
        ...
        <div className="container-custom relative z-10 py-20">
          <div className="max-w-3xl mx-auto text-center">
            ...
            >
              Bitcoin Solutions for Jacksonville Businesses
            </motion.div>
            <motion.h1 
              initial="hidden"
              animate="visible"
          ...
          <div className="grid md:grid-cols-3 gap-8 max-w-5xl mx-auto">
            {pricingPlans.map((plan, index) => (
              <motion.div 
                ...
                className={`card p-8 border ${plan.highlighted ? 'border-bitcoin-orange shadow-lg shadow-bitcoin-orange/10 scale-105 z-10' : 'border-gray-100'}`}
              >
                {plan.highlighted && (
                ...
                <div className="text-bitcoin-blue text-4xl font-bold mb-4">{plan.price}</div>
                <p className="text-gray-500 mb-6">{plan.description}</p>
                <ul className="space-y-3 mb-8">
                  {plan.features.map((feature, featureIndex) => (
                    <li key={featureIndex} className="flex items-start">
                      <CheckCircleIcon className="h-5 w-5 text-bitcoin-orange mr-2 mt-0.5 flex-shrink-0" />
                      <span>{feature}</span>
                    </li>
                  ))}
                </ul>
                <Link href="/services" className={`block text-center px-6 py-3 rounded-full font-medium transition-colors ${plan.highlighted ? 'bg-bitcoin-orange text-white hover:bg-bitcoin-orange/90' : 'bg-white text-bitcoin-blue border border-bitcoin-blue/20 hover:bg-gray-50'}`}>
                  Learn More
                </Link>
              </motion.div>
            ))}
